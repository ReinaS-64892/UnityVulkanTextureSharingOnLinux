// この examples を改変した物です。
// https://github.com/vulkano-rs/vulkano/blob/74d7dc5e3dee9d952fb6904e70b54e63e220fd47/examples/image/main.rs#L575

use rustix::process::getpid;
use std::{error::Error, io::Cursor, mem::forget, os::fd::AsRawFd, sync::Arc};
use vulkano::{
    ...
    memory::{
        DedicatedAllocation, DeviceMemory, ExternalMemoryHandleType, ExternalMemoryHandleTypes, MemoryAllocateInfo, ResourceMemory, allocator::{
...
impl App {
    fn new(event_loop: &EventLoop<()>) -> Self {
        ...
        let texture = {
            // git には image_img.png はないから試したいときには適当な 1024 * 1024 の png を用意してね！(多分透過がないと壊れるかもしれないけど)
            ...
            let image_memory = DeviceMemory::allocate(
                ...
                MemoryAllocateInfo {
                    ...
                    // ここで raw_image を指定しつつ、 export_handle_types に fd をしているすることで export_fd が使えるようになるのだと思しき
            ...
            println!(
                "pid-fd:{} {}",
                getpid().as_raw_pid(),
                image_fd.as_raw_fd() as i32
            );

            // あくまで File 構造体になっているので File Descriptor の解放が行われてしまうと困るので、 forget して解放を絶対にしないようにする。
            forget(image_fd);
...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        let window = Arc::new(
            event_loop
                .create_window(Window::default_attributes().with_title("vulkano fd send"))
                .unwrap(),
        );
